...
if system=='Windows':
	...
	#env.AppendUnique(CXXFLAGS=["/MD","/EHsc","/MACHINE x86"])
	env.AppendUnique(CXXFLAGS=["/MD","/EHsc"])
	#build lib file
	#--imgui
	env.Library(buildroot + os.sep +  'imgui',Glob("api\\imgui"+ os.sep +  '*.cpp')) #Imgui
	env.Library(buildroot + os.sep +  'stb',Glob("api\\stb"+ os.sep +  '*.c')) #Imgui
	#env.Library(buildroot + os.sep +  'glfw',Glob("api\\glfw"+ os.sep +  '*.cpp')) #Imgui
	#env.Library(buildroot + os.sep +  'glad',Glob("api\\glad\\include\\"+ os.sep +  '*.cpp')) #Imgui
	...
	#exeute application 
	#env.Program(targetpath, Glob('scr' + os.sep + '*.cpp'), LIBS=lib_packages, LIBPATH=['.',SFML_LIBS, buildroot])
	env.Program(targetpath, Glob('src' + os.sep + 'main.cpp'), LIBS=lib_packages, LIBPATH=['.','src', buildroot])
